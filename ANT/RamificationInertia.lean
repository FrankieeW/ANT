...
This file provides the computational results for ramification theory in ℤ[√-5].
-/
import ANT.Ideals
...
-- R is already defined in ANT.Ideals as Zsqrtd (-5)
...
The ramification index e(P|p) is the largest n such that p ⊆ P^n.
...
These follow from the ideal factorizations:
- (2) = P2²
- (3) = P3₁ · P3₂
-/
...
theorem ramificationIdx_P2 : -- := 2
  ...
  · intro h
    ...
    have h' : (span ({(2 : R)} : Set R) : Ideal R) * ⊤ ≤ (span ({(2 : R)} : Set R) : Ideal R) * P2 := by
...
The inertia degree f(P|p) is the degree [S/P : R/p] of the residue field extension.
...
These follow from the isomorphism:
- ℤ[√-5]/P2 ≅ ℤ/2ℤ
- ℤ[√-5]/P3₁ ≅ ℤ/3ℤ
- ℤ[√-5]/P3₂ ≅ ℤ/3ℤ
-/
...
theorem inertiaDeg_P2 : -- := 1
  ...
  simpa using hfin.trans (by simpa using (Module.finrank_self (ZMod 2)))

/-- The inertia degree of P3₁ over (3) is 1. -/
theorem inertiaDeg_P3₁ : -- := 1
  ...
  simpa using hfin.trans (by simpa using (Module.finrank_self (ZMod 3)))
...
Verification:
- For p = 2: e × f × g = 2 × 1 × 1 = 2 = [ℚ(√-5) : ℚ] ✓
- For p = 3: e₁×f₁ + e₂×f₂ = 1×1 + 1×1 = 2 = [ℚ(√-5) : ℚ] ✓
-/

-- Verify the factorizations are proved
#check factorization_of_two      -- (2) = P2²
#check factorization_of_three    -- (3) = P3₁ · P3₂

-- Verify the ideals are prime
#check isPrime_span_two_one_plus_sqrtd      -- P2 is prime
#check isPrime_span_three_one_plus_sqrtd    -- P3₁ is prime
#check isPrime_span_three_one_minus_sqrtd   -- P3₂ is prime
